...
export default function DashboardSection() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [
          medicineRes, groupRes, outOfStockResponse, lowStockResponse,
          custRes, empRes, medicinesSoldRes, billCountRes, revenueRes
        ] = await Promise.all([
          ...
          fetch('http://localhost:3001/pharmacy/api/total-sales'), // <-- revenue endpoint
        ]);

        const [
          medicineData, groupData, outOfStockData, lowStockData,
          custData, empData, medicinesSoldData, billCountData, revenueData
        ] = await Promise.all([
          medicineRes.json(),
          groupRes.json(),
        ...
        setRevenue(revenueData.grandTotal || 0); // <-- set revenue
        setLoading(false);
      } catch (err) {
  ...
  const overviewCards = [
    ...
    },
    {
      ...
      title: `Rs. ${revenue.toLocaleString()}`, // <-- format revenue
      subtitle: "Revenue",
      buttonText: "View Detailed Report",
