...
router.post("/empregister",validEMPRegister, async (req, res) => {
    ...
    const {first_name, last_name, email, phone_number, date_of_birth, gender, role, address, password} =req.body;
    ...
    try {
        ...
        db.query('SELECT * FROM employee WHERE email = ?', [email], async (err, results) => {
            ...
            db.query(emp,[first_name,last_name,email,phone_number,role,date_of_birth,gender,address,hashedPassword], (err, ownerResult) => {
                if(err) {
...
router.get("/employees", async (req, res) => {
  ...
  router.put("/empupdate", async (req, res) => {
    ...
    try {
      const { first_name,last_name,email,phone_number,role,date_of_birth,gender,address, } = req.body;
      const empsql =
        "UPDATE employee SET first_name = ?, last_name = ?, email = ?, phone_number = ?, role = ?, date_of_birth = ?, gender = ?, address = ? WHERE employee_id = ?";
  
      db.query(
        empsql,
        [first_name, last_name, email, phone_number, role, date_of_birth, gender, address, employee_id],
        (err, results) => {
          if (err) {
