...
transporter.verify((error, success) => {
    if (error) {
        ...
        console.log('Mail transporter for deworming is ready');
    }
});
...
const processPendingDewormingNotifications = async () => {
    ...
    try {
        const [templates, allPets, allSentDeworming] = await Promise.all([
            query('SELECT * FROM dog_deworm_templates'),
...
router.post('/trigger-deworming-notifications', async (req, res) => {
    try {
...
const dailyDewormingCheck = () => {
    ...
    // Schedule to run at 8:05 AM daily to avoid overlapping with the other service
    cron.schedule('5 8 * * *', () => {
